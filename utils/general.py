...
def pure_torch_nms(boxes, scores, iou_thres):
    if boxes.numel() == 0:
    ...
    if torchvision is not None:
        try:
            ...
            logging.getLogger(__name__).warning(
                "torchvision.ops.nms unavailable, falling back to pure PyTorch NMS: %s",
                exc,
            )
    elif _TORCHVISION_IMPORT_ERROR is not None:
        logging.getLogger(__name__).warning(
            "torchvision import failed, falling back to pure PyTorch NMS: %s",
            _TORCHVISION_IMPORT_ERROR,
        )
    return pure_torch_nms(boxes, scores, iou_thres)
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = safe_nms(boxes, scores, iou_thres)  # NMS
